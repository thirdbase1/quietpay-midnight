...
export const Board: React.FC<Readonly<BoardProps>> = ({ boardDeployment$ }) => {
  ...
  const onFund = useCallback(
    () => runCircuit('Funding', () => deployedBoardAPI!.fund(BigInt(amountPrompt))),
    [runCircuit, deployedBoardAPI, amountPrompt],
  );
  const onPostRoot = useCallback(
    () => runCircuit('Payroll root posted', () => deployedBoardAPI!.postRoot(Buffer.from(rootPrompt, 'hex'))),
    [runCircuit, deployedBoardAPI, rootPrompt],
  );
  const onClaim = useCallback(
    () => runCircuit('Claim', () => deployedBoardAPI!.claim()),
    [runCircuit, deployedBoardAPI],
  );
  const onNextRound = useCallback(
    () => runCircuit('Next round opened', () => deployedBoardAPI!.nextRound()),
    [runCircuit, deployedBoardAPI],
  );
      ...
      {boardDeployment$ && (
        <React.Fragment>
          ...
          <CardContent>
            {boardState ? (
              <React.Fragment>
                <Typography color="primary" sx={{ fontVariantNumeric: 'tabular-nums' }}>
                  Total funded: {boardState.totalFunded.toString()}
                </Typography>
                <Typography color="primary" sx={{ fontVariantNumeric: 'tabular-nums' }}>
                  Total claimed: {boardState.totalClaimed.toString()}
                </Typography>
                ...
                <TextField
                  ...
                  value={amountPrompt}
                  onChange={(e) => setAmountPrompt(e.target.value)}
                />
                <TextField
                  data-testid="payroll-root"
                  variant="outlined"
                  fullWidth
                  size="small"
                  placeholder="Payroll root hex"
                  value={rootPrompt}
          ...
          <CardActions>
            {deployedBoardAPI ? (
              <React.Fragment>
                <Button variant="contained" color="primary" data-testid="fund-btn" onClick={onFund}>
                  Fund
                </Button>
                <Button variant="outlined" data-testid="post-root-btn" onClick={onPostRoot}>
                  Post root
                </Button>
                <Button variant="contained" color="secondary" data-testid="claim-btn" onClick={onClaim}>
                  Claim
                </Button>
                <Button variant="outlined" data-testid="prove-btn" onClick={onProve}>
                  Prove
                </Button>
                <Button variant="outlined" data-testid="next-round-btn" onClick={onNextRound}>
                  Next
                </Button>
              </React.Fragment>
            ) : (
