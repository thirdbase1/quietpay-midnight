 ...
 * `DeployedQuietPayAPI` instance, upon which it will then subscribe to its `state$` observable in order
...
export const Board: React.FC<Readonly<BoardProps>> = ({ boardDeployment$ }) => {
  ...
  // state, and we just need to forward it to the `post` method of the `DeployedQuietPayAPI` instance
  ...
  // of the `DeployedQuietPayAPI` instance.
  ...
  // Subscribes to the `state$` observable on a `DeployedQuietPayAPI` if we receive one, allowing the
